...
class DownloadRepository @Inject constructor() {
    ...
    /**
     * Verifies the format string resolves for a given URL without downloading anything.
     * Throws if the format is unavailable — same failure mode as downloadVideo but fast.
     */
    suspend fun checkFormat(videoUrl: String, config: PlaylistConfig) =
        withContext(Dispatchers.IO) {
            val request = YoutubeDLRequest(videoUrl).apply {
                addOption("-f", buildFormatString(config))
                addOption("--simulate")
                addOption("--no-playlist")
                addOption("--no-warnings")
                addOption("--socket-timeout", "30")
